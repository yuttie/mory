...
fn an_all_day_series_bounded_by_a_date_keeps_every_occurrence() {
    ...
    // start is then anchored to the feed's own zone on the way into `rrule`, so the cut-off has to
    // be anchored with it -- left floating, the rule is refused and the whole series disappears.
    let calendar = calendar_of(
        "BEGIN:VEVENT\r\nDTSTART;VALUE=DATE:20230417\r\nDTEND;VALUE=DATE:20230418\r\n\
         RRULE:FREQ=MONTHLY;UNTIL=20250421;INTERVAL=12;BYDAY=3MO\r\n\
         UID:bounded@example\r\nSUMMARY:Third Monday each April\r\nEND:VEVENT\r\n",
    );
    let (from, to) = window("2023-01-01", "2026-01-01");
    let expansion = crate::ical::expand(&calendar, "cal", from, to);

    assert!(expansion.warnings.is_empty(), "{:?}", expansion.warnings);
    // The `UNTIL` date itself still occurs, as it does in Google's own view of this series.
    assert_eq!(starts(&expansion), vec!["2023-04-17", "2024-04-15", "2025-04-21"]);

    // ...and conversion writes the date back exactly as the feed wrote it.
    let repeat = expansion.series["bounded@example"].repeat.as_ref().expect("a rule");
    assert_eq!(repeat.until.as_deref(), Some("2025-04-21"));
}

#[test]
fn a_floating_until_is_read_in_the_zone_its_start_names() {
    let calendar = calendar_of(
        "BEGIN:VEVENT\r\nDTSTART;TZID=Asia/Tokyo:20240401T090000\r\nDURATION:PT1H\r\n\
         RRULE:FREQ=DAILY;UNTIL=20240403T090000\r\n\
         UID:floating-until@example\r\nSUMMARY:Standup\r\nEND:VEVENT\r\n",
    );
    let (from, to) = window("2024-04-01", "2024-04-30");
    let expansion = crate::ical::expand(&calendar, "cal", from, to);

    assert!(expansion.warnings.is_empty(), "{:?}", expansion.warnings);
    // 09:00 on the 3rd is 09:00 in Tokyo, not in UTC, so the third standup is inside the rule.
    assert_eq!(
        starts(&expansion),
        vec![
            "2024-04-01 09:00:00+09:00",
            "2024-04-02 09:00:00+09:00",
            "2024-04-03 09:00:00+09:00",
        ],
    );
}

#[test]
fn a_floating_series_is_left_floating() {
    // Neither a TZID nor a DATE start, so nothing anchors this rule and `rrule` takes the pair as
    // it finds them. Naming the feed's zone here would move every occurrence of a series that
    // deliberately named none.
    let calendar = calendar_of(
        "BEGIN:VEVENT\r\nDTSTART:20240401T090000\r\nRRULE:FREQ=DAILY;UNTIL=20240403T090000\r\n\
         UID:floating@example\r\nSUMMARY:Wherever you are\r\nEND:VEVENT\r\n",
    );
    let (from, to) = window("2024-04-01", "2024-04-30");
    let expansion = crate::ical::expand(&calendar, "cal", from, to);
